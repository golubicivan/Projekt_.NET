...
namespace ZagrebEvents.Web.Controllers
{
    public class PriceListItemController : Controller
    {
        ...
        {
            if (!CanManageVenueId(item.VenueId)) return Forbid();
            if (!ModelState.IsValid) return View(item);
            _db.PriceListItems.Add(item);
            _db.SaveChanges();
            TempData["Success"] = $"Stavka '{item.ItemName}' dodana u cjenik.";
            return RedirectToAction("Details", "Venue", new { id = item.VenueId });
        }
        ...
        public IActionResult Edit(int id)
        {
            ...
            if (!CanManageVenueId(p.VenueId)) return Forbid();
            return View(p);
        }
        ...
        public async Task<IActionResult> EditPost(int id)
        {
            ...
            return RedirectToAction("Details", "Venue", new { id = p.VenueId });
        }
